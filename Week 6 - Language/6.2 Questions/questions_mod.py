...
def main():
    ...
    if len(sys.argv) == 1:
        # load data
        try:
            files = joblib.load("files.pkl")
            file_words = joblib.load("file_words.pkl")
            file_idfs = joblib.load("file_idfs.pkl")
        except:
            sys.exit("Usage: python questions_mod.py corpus")
    elif len(sys.argv) == 2:
    # Calculate IDF values across files
        ...
        joblib.dump(file_words, "file_words.pkl")
        joblib.dump(file_idfs, "file_idfs.pkl")
    else:
        sys.exit("Usage: python questions_mod.py corpus")
